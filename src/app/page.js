    ...
    <div className='container mx-auto px-4 py-8'>
      ...
      <div className='flex flex-col md:flex-row md:space-x-4 space-y-2 md:space-y-0 mb-6'>
        <input
          ...
          type='text'
          placeholder='Tìm theo Tên Zalo'
          value={filterZalo}
          onChange={(e) => setFilterZalo(e.target.value)}
        />
        <input
          className='border p-2 rounded-md bg-gray-700 text-white w-full md:w-1/2'
          type='text'
          placeholder='Tìm theo Tên Ingame'
          value={filterIngame}
          onChange={(e) => setFilterIngame(e.target.value)}
        />
      </div>
      ...
      >
        <div className='grid grid-cols-1 gap-3'>
          <input
            className='border p-2 rounded-md bg-gray-700 text-white w-full'
            type='text'
      ...
      {loading ? (
        <div className='space-y-4'>
          {Array.from({ length: 10 }).map((_, index) => (
            <div
              key={index}
              className='animate-pulse bg-gray-700 p-4 rounded-md w-full h-12'
            ></div>
          ))}
        </div>
      ) : (
